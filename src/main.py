...
def main():
    ...
    formatted_repo_info = retrieve_github_repo_info(config.repo_url, token=config.github_token)

    # Save the formatted information to a file
    output_file_name = config.data_path / f"{config.repo_url.split('/')[-1]}-formatted-prompt.txt"
